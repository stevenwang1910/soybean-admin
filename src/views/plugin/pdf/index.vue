...
async function generateThumbnails() {
  ...
  // 创建一个数组来存储所有页面的缩略图生成任务
  const thumbnailPromises = [];

  for (let i = 1; i <= pdfDoc.numPages; i++) {
    // 为每个页面创建一个生成缩略图的任务
    const thumbnailPromise = (async (pageNum) => {
      try {
        // 加载指定页面
  ...
  // 将生成的缩略图存储到thumbnails数组中
  thumbnails.value = generatedThumbnails.filter(thumbnail => thumbnail !== null);
}
  ...
  <div class="overflow-hidden h-full">
    <NCard title="PDF 预览" :bordered="false" class="h-full card-wrapper" content-class="overflow-hidden">
      <div class="h-full flex-col-stretch">
        ...
        <div class="flex-1-hidden flex gap-12px">
          <!-- PDF 预览区域 -->
          ...
          <NScrollbar v-if="showThumbnails && thumbnails.length > 0" class="w-64 bg-gray-50">
            <div class="p-4">
              <h3 class="text-sm font-medium mb-4">页面缩略图</h3>
              <div class="space-y-4">
                <div
                  v-for="(thumbnail, index) in thumbnails"
                  :key="index"
                  class="cursor-pointer rounded border-2 transition-all hover:border-primary hover:shadow-md"
                  :class="{
